			...
			<div class="board_view">
				...
				<div class="info">
					<dl>
						...
						<dd>
							<button
								onclick="window.open('/insertMsg.do?memId=<%=bv.getMemId()%>',
					...
					<dl>
						<dt>
							<a
								href="./boardReportInsert.do?bdNo=<%=bv.getBdNo() %>&memId=<%=bv.getMemId()%>"
								id="bt_but" style="font-style: oblique; color: crimson;">신고하기</a>
						</dt>
					</dl>
				</div>
				<div class="cont" id="board_cont" style="text-align: center;">
					<p><%=bv.getBdCont()%></p>
					<%
					if (atchFileList != null) {
				...
				%>
				<a
					href="<%=request.getContextPath()%>/board/download.do?bafId=<%=fileVO.getBafId()%>&bafdNo=<%=fileVO.getBafdNo()%>"
					id="img_download"><%=fileVO.getBafdOrinm()%></a>
				<%
}
}
%>
...
%>	
				<a href="./freeUpdate.do?bdNo=<%=bv.getBdNo()%>">게시글 수정</a> 
				<a href="./freeDelete.do?bdNo=<%=bv.getBdNo()%>">게시글 삭제</a>
		 	</div>
<%
	}
%>
...
<%
	if(memId != null && memId.equals(cv.getMemId())){
%>	
